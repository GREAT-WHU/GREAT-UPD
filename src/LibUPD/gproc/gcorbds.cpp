...
namespace great {
	...
	t_gcorbds::t_gcorbds(t_gsetbase* settings)
	{
		...
		_proc_band = dynamic_cast<t_gsetgnss*>(settings)->band(BDS);
	...
	void t_gcorbds::apply_IGSO_MEO(const t_gtime now, t_gtriple& rec_crd, t_gallnav* gnav, vector<t_gobsgnss>& obsdata)
	{
		if (!this->_correct_bds_code_bias || obsdata.size() == 0) return;

		if (rec_crd.zero())
		{
			if (!this->_recAprCoordinate(now, rec_crd, gnav, obsdata)) return;
		}
		...
		{
			if (data->getrangestate("bds_code_bias"))    // 已改bds_code_bias
			{
				data++;
				continue;
			}
			if (data->gsys() != BDS)  // 同BDS
			{
				data++;
				continue;
			}
			...
			for (auto obs_type : obs_vec)
			{
				...
				//cout << data->epoch().str_ymdhms() << "   " << sat << "   " << gobs2str(obs_type) << fixed << setw(16) << setprecision(3) << obs_P
				//	<< setw(16) << setprecision(3) << Band_cor[b] << setw(16) << setprecision(3) << obs_P + Band_cor[b] << endl;
	...
	bool t_gcorbds::_recAprCoordinate(const t_gtime& now, t_gtriple& rec_crd, t_gallnav* gnav, vector<t_gobsgnss>& obsdata)
	{
		Matrix BB;
		...
		for (iter = obsdata.begin(); iter != obsdata.end();)
		{
			...
			GOBSBAND b1 = _proc_band[0];
			GOBSBAND b2 = _proc_band[1];
			...
			if (!this->_satPosClk(now, sat, gnav, P3, sat_pos, sat_clk))
			{
	...
	bool t_gcorbds::_satPosClk(const t_gtime& now, string sat, t_gallnav* gnav, double P3, t_gtriple& sat_pos, double& sat_clk)
	{
